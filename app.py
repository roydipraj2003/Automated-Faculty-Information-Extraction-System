...
st.title("Faculty Data Scraper and Viewer")

college_name = st.text_input("Enter College Name:")

mode = st.radio("Select input mode:", ["Upload Excel", "Scrape from URL"])

df = None

if mode == "Upload Excel":
    uploaded_file = st.file_uploader("Upload Faculty Excel file", type=["xlsx", "xls"])
    if uploaded_file:
        df = pd.read_excel(uploaded_file)
        st.success("Excel file loaded successfully!")
elif mode == "Scrape from URL":
    url = st.text_input("Enter Faculty Page URL:")
    if st.button("Scrape Faculty Data"):
        if not college_name:
            st.error("Please enter the college name first.")
        elif not url:
            st.error("Please enter the faculty page URL.")
        else:
            with st.spinner("Scraping faculty data..."):
                df = scrape_faculty(url, college_name)
                st.success(f"Scraped {len(df)} faculty members.")
